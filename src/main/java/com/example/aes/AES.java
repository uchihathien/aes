package com.example.aes;

public class AES {

    private static final int nb =4;
    private static final int nk =4;
    private static final int nr =10;

    private static final int[] S_BOX = new int[] {
            
    };
    private static final int[] INV_S_BOX = new int[] {
            
    };



    private static void subBytes(int[][] state){
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < nb; col++) {
                state[row][col] = S_BOX[state[row][col]];
            }
        }
    }
    private static void invSubBytes(int[][] state) {
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < nb; col++) {
                state[row][col] = INV_S_BOX[state[row][col]];
            }
        }
    }
    private static void invShiftRows(int[][] state) {
        for (int row = 1; row < 4; row++) {
            int[] temp = new int[nb];
            for (int col = 0; col < nb; col++) {
                temp[(col + row) % nb] = state[row][col];
            }
            System.arraycopy(temp, 0, state[row], 0, nb);
        }
    }

    private static void invMixColumns(int[][] state) {
        for (int col = 0; col < nb; col++) {
            int[] temp = new int[4];
            for (int row = 0; row < 4; row++) {
                temp[row] = state[row][col];
            }
            // Thực hiện nhân ma trận với các giá trị ngược trong AES
            state[0][col] = gmul(temp[0], 0x0e) ^ gmul(temp[1], 0x0b) ^ gmul(temp[2], 0x0d) ^ gmul(temp[3], 0x09);
            state[1][col] = gmul(temp[0], 0x09) ^ gmul(temp[1], 0x0e) ^ gmul(temp[2], 0x0b) ^ gmul(temp[3], 0x0d);
            state[2][col] = gmul(temp[0], 0x0d) ^ gmul(temp[1], 0x09) ^ gmul(temp[2], 0x0e) ^ gmul(temp[3], 0x0b);
            state[3][col] = gmul(temp[0], 0x0b) ^ gmul(temp[1], 0x0d) ^ gmul(temp[2], 0x09) ^ gmul(temp[3], 0x0e);
        }
    }

    private static void shiftRows(int[][] state) {
        for (int row = 1; row < 4; row++) {
            int[] temp = new int[nb];
            for (int col = 0; col < nb; col++) {
                temp[col] = state[row][(col + row) % nb];
            }
            System.arraycopy(temp, 0, state[row], 0, nb);
        }
    }


    private static void mixColumns(int[][] state) {
        for (int col = 0; col < nb; col++) {
            int[] temp = new int[4];
            for (int row = 0; row < 4; row++) {
                temp[row] = state[row][col];
            }
            // Thực hiện nhân ma trận với các giá trị cố định của AES
            state[0][col] = gmul(temp[0], 2) ^ gmul(temp[1], 3) ^ temp[2] ^ temp[3];
            state[1][col] = temp[0] ^ gmul(temp[1], 2) ^ gmul(temp[2], 3) ^ temp[3];
            state[2][col] = temp[0] ^ temp[1] ^ gmul(temp[2], 2) ^ gmul(temp[3], 3);
            state[3][col] = gmul(temp[0], 3) ^ temp[1] ^ temp[2] ^ gmul(temp[3], 2);
        }
    }


    private static int gmul(int a, int b) {
        int p = 0;
        for (int i = 0; i < 8; i++) {
            if ((b & 1) != 0) p ^= a;
            boolean highBit = (a & 0x80) != 0;
            a <<= 1;
            if (highBit) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    }

    private static void addRoundKey(int[][] state, int[][] roundKey) {
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < nb; col++) {
                state[row][col] ^= roundKey[row][col];
            }
        }
    }

    public static void aesEncrypt(int[][] state, int[][] keySchedule) {
        addRoundKey(state, keySchedule);  // Vòng đầu tiên chỉ AddRoundKey

        for (int round = 1; round < nr; round++) {
            subBytes(state);
            shiftRows(state);
            mixColumns(state);
            addRoundKey(state, keySchedule);
        }

        subBytes(state);  // Vòng cuối không có MixColumns
        shiftRows(state);
        addRoundKey(state, keySchedule);
    }

    public static void aesDecrypt(int[][] state, int[][] keySchedule) {
        addRoundKey(state, keySchedule);  // Vòng đầu tiên chỉ AddRoundKey

        for (int round = nr - 1; round >= 1; round--) {
            invShiftRows(state);
            invSubBytes(state);
            addRoundKey(state, keySchedule);
            invMixColumns(state);
        }

        invShiftRows(state);  // Vòng cuối không có MixColumns
        invSubBytes(state);
        addRoundKey(state, keySchedule);
    }


}